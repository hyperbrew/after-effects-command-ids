var s = {
  "After Effects": [
    "About After Effects",
    {
      "Settings…": [
        "General",
        "Previews",
        "Display",
        "Import",
        "Output",
        "Grids & Guides",
        "Labels",
        "Media & Disk Cache",
        "Video Preview",
        "Appearance",
        "New Project",
        "Auto-Save",
        "Memory & Performance",
        "Audio Hardware",
        "Audio Output Mapping",
        "Sync Settings",
        "Type",
        "Scripting & Expressions",
        "3D",
        "Notifications",
      ],
    },
    {
      "Sync Settings": [
        "Sync Settings Now",
        "Last Update: ",
        "Clear Settings",
        "Manage Sync Settings",
      ],
    },
    {
      Services: [
        "Development",
        "Activity Monitor",
        "Allocations & Leaks",
        "File Activity",
        "System Trace",
        "Time Profile Active Application",
        "Time Profile App Under Mouse",
        "Time Profile Entire System",
        "Toggle Instruments Recording",
        "Services Settings…",
      ],
    },
    "Hide After Effects",
    "Hide Others",
    "Show All",
    "Quit After Effects",
  ],
  File: [
    {
      New: [
        "New Project",
        "New Team Project",
        "New Folder",
        "Adobe Photoshop File",
        "Maxon Cinema 4D File",
      ],
    },
    "Open Project",
    "Open Team Project",
    "Browse in Bridge",
    "Close",
    "Close Project",
    "Save",
    {
      "Save As": [
        "Save As",
        "Save a Copy",
        "Save a Copy As XML",
        "Save a Copy As 18.x",
        "Save a Copy As 17.x",
      ],
    },
    "Increment and Save",
    "Revert",
    {
      Import: [
        "File",
        "Multiple Files",
        "From Libraries",
        "Import Adobe Premiere Pro Project",
        "Vanishing Point (.vpe)",
        "Placeholder",
        "Solid",
      ],
    },
    {
      Export: [
        "Add to Adobe Media Encoder Queue",
        "Add to Render Queue",
        "Export Adobe Premiere Pro Project",
        "Maxon Cinema 4D Exporter",
      ],
    },
    "Add Fonts from Adobe",
    {
      "Adobe Dynamic Link": [
        "New Premiere Pro Sequence",
        "Import Premiere Pro Sequence",
      ],
    },
    "Find",
    "Add Footage to Comp",
    "New Comp from Selection",
    {
      Dependencies: [
        "Collect Files",
        "Consolidate All Footage",
        "Remove Unused Footage",
        "Reduce Project",
        "Find Missing Effects",
        "Find Missing Fonts",
        "Find Missing Footage",
      ],
    },
    "Watch Folder",
    { "Create Proxy": ["Still", "Movie"] },
    { "Set Proxy": ["File", "None"] },
    {
      "Interpret Footage": [
        "Main",
        "Proxy",
        "Remember Interpretation",
        "Apply Interpretation",
      ],
    },
    {
      "Replace Footage": ["File", "With Layered Comp", "Placeholder", "Solid"],
    },
    "Reload Footage",
    "License",
    "Reveal in Finder",
    "Reveal in Bridge",
    "Project Settings",
  ],
  Edit: [
    "Can't Undo",
    "Can't Redo",
    "Cut",
    "Copy",
    "Copy with Property Links",
    "Copy with Relative Property Links",
    "Copy Expression Only",
    "Paste",
    "Clear",
    "Duplicate",
    "Split Layer",
    "Lift Work Area",
    "Extract Work Area",
    "Select All",
    "Deselect All",
    {
      Label: [
        "Select Label Group",
        "None",
        "Red",
        "Yellow",
        "Aqua",
        "Pink",
        "Lavender",
        "Peach",
        "Sea Foam",
        "Blue",
        "Green",
        "Purple",
        "Orange",
        "Brown",
        "Fuchsia",
        "Cyan",
        "Sandstone",
        "Dark Green",
        "Edit Label Colors",
      ],
    },
    {
      "Select Keyframe Label Group": [
        "On Selected Layers",
        "On All Layers",
        "Visible Keyframes on Selected Layers",
        "Visible Keyframes on All Layers",
      ],
    },
    {
      Purge: [
        "All Memory & Disk Cache",
        "All Memory",
        "Undo",
        "Image Cache Memory",
        "Snapshot",
      ],
    },
    "Edit Original",
    "Edit in Adobe Audition",
    {
      "Team Project": [
        "Get Latest Changes",
        "Share My Changes",
        "Resolve Conflicts",
        "Convert Team Project to Project",
        "Media Management",
      ],
    },
    { Templates: ["Render Settings", "Output Module"] },
    "Keyboard Shortcuts",
    "Paste Mocha mask",
    "Start Dictation…",
  ],
  Composition: [
    "New Composition",
    "Composition Settings",
    "Set Poster Time",
    "Trim Comp to Work Area",
    "Crop Comp to Region of Interest",
    "Add to Adobe Media Encoder Queue",
    "Add to Render Queue",
    "Add Output Module",
    {
      Preview: ["Play Current Preview", "Cache Frames When Idle", "Audio"],
    },
    {
      "Save Frame As": ["File", "Photoshop Layers", "ProEXR"],
    },
    "Pre-render",
    "Save Current Preview",
    "Open in Essential Graphics",
    {
      "Responsive Design — Time": [
        "Create Intro",
        "Create Outro",
        "Create Protected Region from Work Area",
      ],
    },
    "Composition Flowchart",
    "Composition Mini-Flowchart",
    { VR: ["Create VR Environment", "Extract Cubemap"] },
  ],
  Layer: [
    {
      New: [
        "Text",
        "Solid",
        "Light",
        "Camera",
        "Null Object",
        "Shape Layer",
        "Adjustment Layer",
        "Content-Aware Fill Layer",
        "Adobe Photoshop File",
        "Maxon Cinema 4D File",
      ],
    },
    "Solid Settings",
    "Open Layer",
    "Open Layer Source",
    "Reveal in Finder",
    {
      Mask: [
        "New Mask",
        "Mask Shape",
        "Mask Feather",
        "Mask Opacity",
        "Mask Expansion",
        "Reset Mask",
        "Remove Mask",
        "Remove All Masks",
        {
          Mode: [
            "None",
            "Add",
            "Subtract",
            "Intersect",
            "Lighten",
            "Darken",
            "Difference",
          ],
        },
        "Inverted",
        "Locked",
        { "Motion Blur": ["Same As Layer", "On", "Off"] },
        { "Feather Falloff": ["Smooth", "Linear"] },
        "Unlock All Masks",
        "Lock Other Masks",
        "Hide Locked Masks",
      ],
    },
    {
      "Mask and Shape Path": [
        "RotoBezier",
        "Closed",
        "Convert To Bezier Path",
        "Group Shapes",
        "Ungroup Shapes",
        "Set First Vertex",
        "Free Transform Points",
      ],
    },
    {
      Quality: ["Best", "Draft", "Wireframe", "Bilinear", "Bicubic"],
    },
    {
      Switches: [
        "Hide Other Video",
        "Show All Video",
        "Unlock All Layers",
        "Enable Expressions",
        "Disable Expressions",
        "Shy",
        "Lock",
        "Audio",
        "Video",
        "Solo",
        "Effect",
        "Collapse",
        "Motion Blur",
        "Adjustment Layer",
      ],
    },
    {
      Transform: [
        "Reset",
        "Anchor Point",
        "Position",
        "Scale",
        "Orientation",
        "Rotation",
        "Opacity",
        "Flip Horizontal",
        "Flip Vertical",
        "Center In View",
        "Center Anchor Point in Layer Content",
        "Fit to Comp",
        "Fit to Comp Width",
        "Fit to Comp Height",
        "Auto-Orient",
      ],
    },
    {
      Time: [
        "Enable Time Remapping",
        "Time-Reverse Layer",
        "Time Stretch",
        "Freeze Frame",
        "Freeze On Last Frame",
        "Align Video to Data",
      ],
    },
    { "Frame Blending": ["Off", "Frame Mix", "Pixel Motion"] },
    "3D Layer",
    "Guide Layer",
    "Environment Layer",
    {
      Markers: [
        "Add Marker",
        "Update Markers From Source",
        "Lock Markers",
        "Delete All Markers",
      ],
    },
    "Preserve Transparency",
    {
      "Blending Mode": [
        "Normal",
        "Dissolve",
        "Dancing Dissolve",
        "Darken",
        "Multiply",
        "Color Burn",
        "Classic Color Burn",
        "Linear Burn",
        "Darker Color",
        "Add",
        "Lighten",
        "Screen",
        "Color Dodge",
        "Classic Color Dodge",
        "Linear Dodge",
        "Lighter Color",
        "Overlay",
        "Soft Light",
        "Hard Light",
        "Linear Light",
        "Vivid Light",
        "Pin Light",
        "Hard Mix",
        "Difference",
        "Classic Difference",
        "Exclusion",
        "Subtract",
        "Divide",
        "Hue",
        "Saturation",
        "Color",
        "Luminosity",
        "Stencil Alpha",
        "Stencil Luma",
        "Silhouette Alpha",
        "Silhouette Luma",
        "Alpha Add",
        "Luminescent Premul",
      ],
    },
    "Next Blending Mode",
    "Previous Blending Mode",
    {
      "Track Matte": [
        "No Track Matte",
        "Alpha Matte",
        "Alpha Inverted Matte",
        "Luma Matte",
        "Luma Inverted Matte",
      ],
    },
    {
      "Layer Styles": [
        "Convert to Editable Styles",
        "Show All",
        "Remove All",
        "Drop Shadow",
        "Inner Shadow",
        "Outer Glow",
        "Inner Glow",
        "Bevel and Emboss",
        "Satin",
        "Color Overlay",
        "Gradient Overlay",
        "Stroke",
      ],
    },
    {
      Arrange: [
        "Bring Layer to Front",
        "Bring Layer Forward",
        "Send Layer Backward",
        "Send Layer to Back",
      ],
    },
    {
      Reveal: [
        "Reveal in Finder",
        "Reveal Layer Source in Project",
        "Reveal Layer in Project Flowchart",
        "Reveal Composition in Project",
        "Reveal Expression Errors",
      ],
    },
    {
      Create: [
        "Convert to Editable Text",
        "Create Shapes from Text",
        "Create Masks from Text",
        "Create Shapes from Vector Layer",
        "Create Keyframes from Data",
      ],
    },
    {
      Camera: [
        "Create Camera from 3D View",
        "Create Stereo 3D Rig",
        "Create Orbit Null",
        "Link Focus Distance to Point of Interest",
        "Link Focus Distance to Layer",
        "Set Focus Distance to Layer",
        "Reset Camera 1",
      ],
    },
    "Auto-trace",
    "Pre-compose",
    "Scene Edit Detection",
  ],
  Effect: [
    "Effect Controls",
    "Last Effect",
    "Remove All",
    {
      "3D Channel": [
        "3D Channel Extract",
        "Cryptomatte",
        "Depth Matte",
        "Depth of Field",
        "EXtractoR",
        "Fog 3D",
        "ID Matte",
        "IDentifier",
      ],
    },
    {
      Audio: [
        "Backwards",
        "Bass & Treble",
        "Delay",
        "Flange & Chorus",
        "High-Low Pass",
        "Modulator",
        "Parametric EQ",
        "Reverb",
        "Stereo Mixer",
        "Tone",
      ],
    },
    {
      "Blur & Sharpen": [
        "Bilateral Blur",
        "Camera Lens Blur",
        "Camera-Shake Deblur",
        "CC Cross Blur",
        "CC Radial Blur",
        "CC Radial Fast Blur",
        "CC Vector Blur",
        "Channel Blur",
        "Compound Blur",
        "Directional Blur",
        "Fast Box Blur",
        "Gaussian Blur",
        "Radial Blur",
        "Sharpen",
        "Smart Blur",
        "Unsharp Mask",
      ],
    },
    { "Boris FX Mocha": ["Mocha AE"] },
    {
      Channel: [
        "Arithmetic",
        "Blend",
        "Calculations",
        "CC Composite",
        "Channel Combiner",
        "Compound Arithmetic",
        "Invert",
        "Minimax",
        "Remove Color Matting",
        "Set Channels",
        "Set Matte",
        "Shift Channels",
        "Solid Composite",
      ],
    },
    { "Cinema 4D": ["Cineware"] },
    {
      "Color Correction": [
        "Auto Color",
        "Auto Contrast",
        "Auto Levels",
        "Black & White",
        "Brightness & Contrast",
        "Broadcast Colors",
        "CC Color Neutralizer",
        "CC Color Offset",
        "CC Kernel",
        "CC Toner",
        "Change Color",
        "Change to Color",
        "Channel Mixer",
        "Color Balance",
        "Color Balance (HLS)",
        "Color Link",
        "Color Stabilizer",
        "Colorama",
        "Curves",
        "Equalize",
        "Exposure",
        "Gamma/Pedestal/Gain",
        "Hue/Saturation",
        "Leave Color",
        "Levels",
        "Levels (Individual Controls)",
        "Lumetri Color",
        "Photo Filter",
        "PS Arbitrary Map",
        "Selective Color",
        "Shadow/Highlight",
        "Tint",
        "Tritone",
        "Vibrance",
        "Video Limiter",
      ],
    },
    {
      Distort: [
        "Bezier Warp",
        "Bulge",
        "CC Bend It",
        "CC Bender",
        "CC Blobbylize",
        "CC Flo Motion",
        "CC Griddler",
        "CC Lens",
        "CC Page Turn",
        "CC Power Pin",
        "CC Ripple Pulse",
        "CC Slant",
        "CC Smear",
        "CC Split",
        "CC Split 2",
        "CC Tiler",
        "Corner Pin",
        "Detail-preserving Upscale",
        "Displacement Map",
        "Liquify",
        "Magnify",
        "Mesh Warp",
        "Mirror",
        "Offset",
        "Optics Compensation",
        "Polar Coordinates",
        "Reshape",
        "Ripple",
        "Rolling Shutter Repair",
        "Smear",
        "Spherize",
        "Transform",
        "Turbulent Displace",
        "Twirl",
        "Warp",
        "Warp Stabilizer",
        "Wave Warp",
      ],
    },
    {
      "Expression Controls": [
        "3D Point Control",
        "Angle Control",
        "Checkbox Control",
        "Color Control",
        "Dropdown Menu Control",
        "Layer Control",
        "Point Control",
        "Slider Control",
      ],
    },
    {
      Generate: [
        "4-Color Gradient",
        "Advanced Lightning",
        "Audio Spectrum",
        "Audio Waveform",
        "Beam",
        "CC Glue Gun",
        "CC Light Burst 2.5",
        "CC Light Rays",
        "CC Light Sweep",
        "CC Threads",
        "Cell Pattern",
        "Checkerboard",
        "Circle",
        "Ellipse",
        "Eyedropper Fill",
        "Fill",
        "Fractal",
        "Gradient Ramp",
        "Grid",
        "Lens Flare",
        "Paint Bucket",
        "Radio Waves",
        "Scribble",
        "Stroke",
        "Vegas",
        "Write-on",
      ],
    },
    {
      "Immersive Video": [
        "VR Blur",
        "VR Chromatic Aberrations",
        "VR Color Gradients",
        "VR Converter",
        "VR De-Noise",
        "VR Digital Glitch",
        "VR Fractal Noise",
        "VR Glow",
        "VR Plane to Sphere",
        "VR Rotate Sphere",
        "VR Sharpen",
        "VR Sphere To Plane",
      ],
    },
    {
      Keying: [
        "Advanced Spill Suppressor",
        "CC Simple Wire Removal",
        "Color Difference Key",
        "Color Range",
        "Difference Matte",
        "Extract",
        "Inner/Outer Key",
        "Key Cleaner",
        "Keylight (1.2)",
        "Linear Color Key",
      ],
    },
    {
      Matte: [
        "Matte Choker",
        "Refine Hard Matte",
        "Refine Soft Matte",
        "Simple Choker",
      ],
    },
    {
      "Noise & Grain": [
        "Add Grain",
        "Dust & Scratches",
        "Fractal Noise",
        "Match Grain",
        "Median",
        "Median (Legacy)",
        "Noise",
        "Noise Alpha",
        "Noise HLS",
        "Noise HLS Auto",
        "Remove Grain",
        "Turbulent Noise",
      ],
    },
    {
      Obsolete: [
        "Basic 3D",
        "Basic Text",
        "Color Key",
        "Gaussian Blur (Legacy)",
        "Lightning",
        "Luma Key",
        "mocha shape",
        "Path Text",
        "Reduce Interlace Flicker",
        "Spill Suppressor",
      ],
    },
    {
      Perspective: [
        "3D Camera Tracker",
        "3D Glasses",
        "Bevel Alpha",
        "Bevel Edges",
        "CC Cylinder",
        "CC Environment",
        "CC Sphere",
        "CC Spotlight",
        "Drop Shadow",
        "Radial Shadow",
      ],
    },
    { "Sample Plug-ins": ["PathMaster"] },
    {
      Simulation: [
        "Card Dance",
        "Caustics",
        "CC Ball Action",
        "CC Bubbles",
        "CC Drizzle",
        "CC Hair",
        "CC Mr. Mercury",
        "CC Particle Systems II",
        "CC Particle World",
        "CC Pixel Polly",
        "CC Rainfall",
        "CC Scatterize",
        "CC Snowfall",
        "CC Star Burst",
        "Foam",
        "Particle Playground",
        "Shatter",
        "Wave World",
      ],
    },
    {
      Stylize: [
        "Brush Strokes",
        "Cartoon",
        "CC Block Load",
        "CC Burn Film",
        "CC Glass",
        "CC HexTile",
        "CC Kaleida",
        "CC Mr. Smoothie",
        "CC Plastic",
        "CC RepeTile",
        "CC Threshold",
        "CC Threshold RGB",
        "CC Vignette",
        "Color Emboss",
        "Emboss",
        "Find Edges",
        "Glow",
        "Mosaic",
        "Motion Tile",
        "Posterize",
        "Roughen Edges",
        "Scatter",
        "Strobe Light",
        "Texturize",
        "Threshold",
      ],
    },
    { Text: ["Numbers", "Timecode"] },
    {
      Time: [
        "CC Force Motion Blur",
        "CC Wide Time",
        "Echo",
        "Pixel Motion Blur",
        "Posterize Time",
        "Time Difference",
        "Time Displacement",
        "Timewarp",
      ],
    },
    {
      Transition: [
        "Block Dissolve",
        "Card Wipe",
        "CC Glass Wipe",
        "CC Grid Wipe",
        "CC Image Wipe",
        "CC Jaws",
        "CC Light Wipe",
        "CC Line Sweep",
        "CC Radial ScaleWipe",
        "CC Scale Wipe",
        "CC Twister",
        "CC WarpoMatic",
        "Gradient Wipe",
        "Iris Wipe",
        "Linear Wipe",
        "Radial Wipe",
        "Venetian Blinds",
      ],
    },
    {
      Utility: [
        "Apply Color LUT",
        "CC Overbrights",
        "Cineon Converter",
        "Color Profile Converter",
        "Grow Bounds",
        "HDR Compander",
        "HDR Highlight Compression",
      ],
    },
  ],
  Animation: [
    "Save Animation Preset",
    "Apply Animation Preset",
    { "Recent Animation Presets": ["Recent Favorites List"] },
    "Browse Presets",
    "Add Keyframe",
    "Toggle Hold Keyframe",
    "Keyframe Interpolation",
    "Keyframe Velocity",
    {
      "Keyframe Assistant": [
        "Convert Audio to Keyframes",
        "Convert Expression to Keyframes",
        "Create Keyframes from Data",
        "Easy Ease",
        "Easy Ease In",
        "Easy Ease Out",
        "Exponential Scale",
        "RPF Camera Import",
        "Sequence Layers",
        "Time-Reverse Keyframes",
      ],
    },
    "Add Property to Essential Graphics",
    {
      "Animate Text": [
        "Enable Per-character 3D",
        "Anchor Point",
        "Position",
        "Scale",
        "Skew",
        "Rotation",
        "Opacity",
        "All Transform Properties",
        { "Fill Color": ["RGB", "Hue", "Saturation", "Brightness", "Opacity"] },
        {
          "Stroke Color": ["RGB", "Hue", "Saturation", "Brightness", "Opacity"],
        },
        "Stroke Width",
        "Tracking",
        "Line Anchor",
        "Line Spacing",
        "Character Offset",
        "Character Value",
        "Blur",
      ],
    },
    { "Add Text Selector": ["Range", "Wiggly", "Expression"] },
    "Remove All Text Animators",
    "Add Expression",
    "Separate Dimensions",
    "Track Camera",
    "Track in Boris FX Mocha",
    "Warp Stabilizer VFX",
    "Track Motion",
    "Track Mask",
    "Track this Property",
    "Reveal Properties with Keyframes",
    "Reveal Properties with Animation",
    "Reveal All Modified Properties",
  ],
  View: [
    "New Viewer",
    "Zoom In",
    "Zoom Out",
    {
      Resolution: ["Full", "Half", "Third", "Quarter", "Custom"],
    },
    "Use Display Color Management		⇧Numpad /",
    {
      "Simulate Output": [
        "No Output Simulation",
        "HDTV (Rec. 709)",
        "SDTV NTSC",
        "SDTV PAL",
        "Legacy Macintosh RGB (Gamma 1.8)",
        "Internet Standard RGB (sRGB)",
        "Kodak 5218 to Kodak 2383",
        "Universal Camera Film to Kodak 2383",
        "My Custom RGB",
        "Custom",
      ],
    },
    "Show Rulers",
    "Show Guides",
    "Snap to Guides",
    "Lock Guides",
    "Clear Guides",
    "Import Guides",
    "Export Guides",
    "Show Grid",
    "Snap to Grid",
    "View Options",
    "Show Layer Controls",
    "Reset Camera 1",
    "Create Camera from 3D View",
    {
      "Switch View Layout": [
        "1 View",
        "2 Views",
        "4 Views",
        "Share View Options",
      ],
    },
    {
      "Switch 3D View": [
        "Active Camera (Camera 1)",
        "Default",
        "Front",
        "Left",
        "Top",
        "Back",
        "Right",
        "Bottom",
        "Custom View 1",
        "Custom View 2",
        "Custom View 3",
        "Reset Camera 1",
      ],
    },
    {
      "Assign Shortcut to “Active Camera (Camera 1)”": [
        "F10 (Replace “Front”)",
        "F11 (Replace “Custom View 1”)",
        "F12 (Replace “Active Camera (Camera 1)”)",
      ],
    },
    "Switch to Last 3D View",
    "Look at Selected Layers",
    "Look at All Layers",
    "Go to Time",
    "Enter Full Screen",
  ],
  Window: [
    {
      Workspace: [
        "All Panels",
        "Animation",
        "Color",
        "Default",
        "Effects",
        "Essential Graphics",
        "Learn",
        "Libraries",
        "Minimal",
        "Motion Tracking",
        "Paint",
        "Review",
        "Small Screen",
        "Standard",
        "Text",
        "Undocked Panels",
        "Reset “All Panels” to Saved Layout",
        "Save Changes to this Workspace",
        "Save as New Workspace",
        "Edit Workspaces",
      ],
    },
    {
      "Assign Shortcut to “All Panels” Workspace": [
        "Shift+F10 (Replace “Default”)",
        "Shift+F11 (Replace “Review”)",
        "Shift+F12 (Replace “Learn”)",
      ],
    },
    "Find Extensions on Exchange",
    {
      Extensions: [
        "Autopod Whisper ",
        "Bolt CEP",
        "Bolt CEP - React",
        "Bolt CEP - Svelte",
        "Bolt CEP - Vue",
        "Code - B",
        "Klutz GPT",
        "NFL Graphics",
        "Pro IO",
        "Pro IO - β",
        "Zap Renamer",
      ],
    },
    "Align",
    "Audio",
    "Brushes",
    "Character",
    "Content-Aware Fill",
    "Effects & Presets",
    "Essential Graphics",
    "Info",
    "Learn",
    "Libraries",
    "Lumetri Scopes",
    "Mask Interpolation",
    "Media Browser",
    "Metadata",
    "Motion Sketch",
    "Paint",
    "Paragraph",
    "Preview",
    "Progress",
    "Review with Frame.io",
    "Smoother",
    "Tools",
    "Tracker",
    "Wiggler",
    "Composition: (none)",
    "Effect Controls: (none)",
    "Flowchart: (none)",
    "Footage: (none)",
    "Layer: (none)",
    "Project",
    "Render Queue",
    "Timeline: (none)",
    "Create Nulls From Paths.jsx",
    "VR Comp Editor.jsx",
  ],
  Help: [
    "After Effects Help",
    "After Effects In-App Tutorials",
    "After Effects Online Tutorials",
    "Scripting Help",
    "Expression Reference",
    "Effect Reference",
    "Animation Presets",
    "Keyboard Shortcuts",
    "System Compatibility Report",
    "Adobe Crash Reporter",
    "Enable Logging",
    "Reveal Logging File",
    "Online User Forums",
    "Provide Feedback",
    "Manage My Account",
    "Sign Out (invertedvisuals@gmail.com)",
    "Updates",
  ],
};

function extractStrings(obj) {
  let result = [];

  const traverse = (value) => {
    if (Array.isArray(value)) {
      value.forEach((item) => traverse(item));
    } else if (typeof value === "object" && value !== null) {
      Object.values(value).forEach((item) => traverse(item));
    } else if (typeof value === "string") {
      result.push(value);
    }
  };

  traverse(obj);

  return result;
}

const strings = extractStrings(s);
console.log(strings); // Outputs all the strings from the nested object

require("fs").writeFileSync(
  "results-flat.json",
  JSON.stringify(strings, null, 2)
);
